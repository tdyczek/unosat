...
class Image:
    ...
    def __repr__(self):
        return f"({self.vv.name}, " f"{self.vh.name})\n"
...
class CityData:
    def __init__(self, city_dir: Path, with_mask=True):
        ...
        self.images = [
            Image(image_dir) for image_dir in city_dir.glob("*") if image_dir.is_dir()
        ]
        if with_mask:
...
def extract_ims(cities_path: Path, with_mask=True):
    ...
    cities = np.array([CityData(city_dir, with_mask) for city_dir in cities_dirs])
    return cities
...
def save_city_mask(city: CityData, out_dir: Path):
    ...
    with rasterio.open(out_dir / f"{city.name}.tif", "w+", **meta) as out:
        ...
        shapes = (
            (geom, value) for geom, value in zip(shp_mask.geometry, shp_mask.positive)
        )
